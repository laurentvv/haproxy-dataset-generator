...
def benchmark_targeted(
    ...
    for i, test in enumerate(questions, 1):
        ...
        # Affichage
        status = "✅" if eval_result["quality_score"] > 0.7 else "⚠️" if eval_result["quality_score"] > 0.4 else "❌"
...
def generate_report(benchmark: dict, output_file: str = "bench_v3_targeted_report.json"):
    ...
    print(f"{'Questions résolues':<30} | {stats['questions_resolues']}/{benchmark['questions_tested']:<14}")
    ...
    print(f"Après (V3) : {stats['avg_quality']:.3f} ({stats['taux_reussite']:.1f}% résolues)")
